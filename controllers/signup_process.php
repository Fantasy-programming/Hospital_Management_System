...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($firstName) || empty($lastName) | empty($userName) || empty($age) || empty($password) || empty($confirmPassword)) {
    ...
    if (count($errors) > 0) {
        ...
        header("Location: user_register.php?error=$errorString");
        exit();
    }
    ...
    $insertQuery = "INSERT INTO patients (first_name, last_name, username, age, role, password) VALUES ('$firstName', '$lastName', '$userName', $age, '$role', '$hashedPassword')";
